...
///////////////////////////////////////////////////////////////////////
// Implemente aqui qualquer funcao adicional que precise
bool intersecao(ARVORE_BINARIA *avr1, TIPOCHAVE chave){
   if (vazia(avr1)){
      return false;
   }
   if (chave == avr1->item.chave){
      return true;
   }
   if (chave < avr1->item.chave){
      return intersecao(avr1->esq, chave);
   }
   else {
      return intersecao(avr1->dir, chave);
   }
}
void cop_elements (ARVORE_BINARIA *origem, ARVORE_BINARIA **destino){
   if (!vazia(origem)){
      cop_elements(origem->esq, destino);
      inserir(origem->item, destino);
      cop_elements(origem->dir, destino);
   }
}
bool tem_intersecao (ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2){
   if (vazia(arv2)){
      return false;
   }
   if (intersecao(arv1, arv2->item.chave)){
      return true;
   }
   return tem_intersecao(arv1, arv2->esq) || tem_intersecao(arv1, arv2->dir);
}
...
/*
 Objetivos: Pegar todos os valores do tipo ITEM contidos nas duas arvores
            passadas como parametro (arv1 e arv2) e fazer uma uniao, colocando
            o resultado no terceiro parametro do tipo ARVORE_BINARIA (arvU).
            Este parametro consiste em uma arvore que deve ser inicializada
            e que deve contar os itens resultantes da uniao, sem compartilhar
            variaveis dinamicas com arv1 e arv2, portanto tratando-se de copia.
            Caso haja alguma intersecao entre as arvores (chaves repetidas),
            a uniao nao deve ser realizada e a funcao uniao retorna false,
            caso contrario, retorna true.
*/
bool uniao(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2, ARVORE_BINARIA **arvU)
{
   *arvU = NULL;

   if (!vazia(arv1) && !vazia(arv2) && tem_intersecao(arv1, arv2)){
      return false;
   }
   cop_elements(arv1, arvU);
   cop_elements(arv2,arvU);
