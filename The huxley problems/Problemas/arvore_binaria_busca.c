...
{
    // Todos os nós da subárvore esquerda têm chave menor que a chave do nó atual.
    //Todos os nós da subárvore direita têm chave maior que a chave do nó atual.
    //Essa propriedade se aplica recursivamente a todos os nós da árvore.

    if (vazia(arvore)){
        return true;
    }
    ARVORE_BINARIA *pilha[100];
    int top = -1 ;
    ARVORE_BINARIA *current = arvore;
    ARVORE_BINARIA *previous = NULL ;
    ...
    while (current != NULL || top >= 0){
        ...
        current = pilha[top--];
        if (previous != NULL && current->item.chave <= previous->item.chave){
            return false;
        }
        previous = current;
        current = current->dir;
