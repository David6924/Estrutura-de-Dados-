...
typedef struct clausula{
    node *literias; // Literias x1 , x2 ,x3 ..... e já vê se ele está negado **/ x1 / /-> / x2 / / -> ...... **
...
formula read_formula (FILE *fp){
    ...
    while (fgets(line, MAX, fp)){
        ...
        int lit; //-1 2 4 ....
        node *lit_head = creat_linked_list(); // NULL
    ...
    while (cl != NULL){  //Ver todas as cláusulas
        ...
        while (lt != NULL){ // Ver os literias x1 x2 ....
            ...
            if (var > 0){ // Se  não tiver negado
               valor = interpretacoes[var - 1] ; // Pegamos o valor no array de atribuição // Reolhar "interpretacoes[var - 1]"" // interpretação[0] "=" 1 // interpretação[1] = "=" 2
            }
            else { // Caso não
                valor = !interpretacoes[-var - 1]; // Vamos negar o valor de atribuição // interpretação[0]  "=" -1
            }
            if (valor){ // Se o valor é verdadeiro
                cl_sat = true;
                break;
            }
            lt = lt->next; // Próximo literal 
        }
...
int main (){
    FILE *fp = fopen("teste_vazio.cnf", "r");
    if (fp == NULL){
